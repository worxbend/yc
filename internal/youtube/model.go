...
// QuotaMode describes why the poll loop is running at its current cadence.
type QuotaMode string

const (
	// QuotaModeLive means the poll cadence is the server-advised floor.
	QuotaModeLive QuotaMode = "live"
	// QuotaModeStretched means the budget floor exceeded the server floor,
	// so yc is polling slower than YouTube allows in order to survive the
	// quota day. The status bar says so explicitly.
	QuotaModeStretched QuotaMode = "stretched"
	// QuotaModeBackoff means an error ladder is in effect.
	QuotaModeBackoff QuotaMode = "backoff"
	// QuotaModePaused means polling stopped: quota exhausted or the reserve
	// threshold tripped.
	QuotaModePaused QuotaMode = "paused"
)

// QuotaSnapshot is an immutable view of the estimated quota ledger plus the
...
func (q QuotaSnapshot) RemainingPercent() float64 {
	if q.LimitUnits <= 0 {
		return 0
	}
...
// stream.
func (q QuotaSnapshot) ProjectedExhaustion(costPerPoll int) (time.Duration, bool) {
